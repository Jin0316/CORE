...
from minigpt4.datasets.datasets.coco_vqa_datasets import SafeErase_Harmimg_Harmtxt, SafeErase_Unharmimg_Unharmtxt
...
if args.harm_combination == 'hh': 
    ...
    data = SafeErase_Harmimg_Harmtxt(vis_processor, image_path, eval_path, keywords)
elif args.harm_combination == 'hu':
    print('[EVAL] Harm Image / Unharm Text')
    data = SafeErase_Harmimg_Unharmtxt(vis_processor, image_path, eval_path, keywords)
elif args.harm_combination == 'uh':
    print('[EVAL] Unharm Image / Harm Text')
    data = SafeErase_Unharmimg_Harmtxt(vis_processor, image_path, eval_path, keywords)
elif args.harm_combination == 'uu':
    print('[EVAL] Unharm Image / Unharm Text')
    data = SafeErase_Unharmimg_Unharmtxt(vis_processor, image_path, eval_path, keywords)

eval_dataloader = DataLoader(data, batch_size=16, shuffle=False)
minigpt4_predict = []
